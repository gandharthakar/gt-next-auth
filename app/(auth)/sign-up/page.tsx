...
function Page() {
    ...
    const [formFildsData, setFormFieldsData] = useState<SignUpFormType>(
        {
            user_full_name: '',
            user_email: '',
            user_password: '',
    ...
    const handleFormSubmit = async (e: any) => {
        ...
        const response = await fetch('/api/user/sign-up', {
            ...
            body: JSON.stringify({
                full_name: formFildsData.user_full_name,
                email: formFildsData.user_email,
                password: formFildsData.user_password,
                confirm_password: formFildsData.user_confirm_password
            })
        });
                ...
                <div className="max-w-[450px] mx-auto w-full">
                    ...
                    <div className="bg-zinc-200 py-[50px] px-[20px] md:px-[50px] rounded-lg">
                        <form onSubmit={handleFormSubmit}>
                            <div className="pb-[15px]">
                                <label htmlFor="fulnam" className="block mb-[5px] text-[18px] md:text-[20px] font-semibold">
                                    Full Name
                                </label>
                                <input 
                                    type="text" 
                                    ...
                                    value={formFildsData.user_full_name}
                                    onChange={handleInputChange}
                                />
                            </div>
                            <div className="pb-[15px]">
                                <label htmlFor="eml" className="block mb-[5px] text-[18px] md:text-[20px] font-semibold">
                                    Email
                                </label>
                                <input 
                                    type="email" 
                            ...
                            <div className="pb-[25px]">
                                <label htmlFor="cnfpaswd" className="block mb-[5px] text-[18px] md:text-[20px] font-semibold">
                                    Confirm Password
                                </label>
                                <input 
                                    type="password" 
                                    ...
                                    autoComplete="off" 
                                    className="transition-all delay-75 block w-full py-[10px] md:py-[15px] px-[20px] border-[2px] border-solid border-zinc-800 bg-white focus:border-blue-700 focus:outline-0 font-semibold text-[16px] md:text-[18px]" 
                                    value={formFildsData.user_confirm_password}
                                    onChange={handleInputChange}
                                />
                            </div>
                    ...
                    <div className="pt-[15px] text-center">
                        <h4 className="text-[16px] md:text-[18px] text-zinc-800">
                            Already have an account ? <span className="font-semibold">Please <Link href="/sign-in" title="Sign In" className="text-blue-700">Sign In</Link></span>
                        </h4>
                    </div>
                </div>
            </div>
